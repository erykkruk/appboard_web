...
/** Format a list of [w, h] pairs as "1290×2796 lub 2796×1290". */
...
/**
 * Build a clear, actionable Polish message from the structured dimension error
 * data the backend returns on a failed upload (or from the pre-upload /validate
 * response when invalid). Names the device, the provided size and the accepted
...
export function buildDimensionMessage(
	...
	return `Nieprawidłowy rozmiar dla ${data.displayTypeName}: podano ${provided}px, oczekiwano ${supported}px.`;
}
